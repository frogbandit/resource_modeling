...
var $axesselection = $('#axes-selection');
var $projectskillselection =  $('#projectskill-selection');
...
function getData() {
    gapi.client.sheets.spreadsheets.values.batchGet({
        ...
        var main_range = range[0];
        var people_range = range[1];
        var project_range = range[2];

        // Get the spreadsheet data into required and provided dictionaries
        if (main_range.values.length > 0) {
            for (var i = 0; i < main_range.values.length; i++) {
                ...
                var salary_list = [];
                var row_num = parseInt(person_id.slice(-2));
                var person_row = people_range.values[row_num];

                if (person_row != undefined) {
                    for (var k = 0; k < person_row.slice(8).length; k++) {
                        salary_list.push(parseFloat(person_row.slice(8)[k].substring(1)) || 0);
                    }
                }
                ...
                var project_row = project_range.values[row_num];

                if (project_row != undefined) {
                    for (k = 0; k < project_row.slice(9).length; k++) {
                        contract_list.push(parseFloat(project_row.slice(9)[k]) || 0);
                    }
                }
                ...
                if (real == '1') {
                    ...
                    /// if Provided, add lists for person, FTE provided, and salary for each skill
                        ...
                        if (project in real_prov_overall_dict) {
                            skill_dict = real_prov_overall_dict[project];
        ...
        var $hypotheticalselection = $('#hypothetical-selection');
        if ($hypotheticalselection.text().trim() == 'Hypothetical On') {
...
function create_total_chart(p, prov, req, skills, months) {
    ...
    if ($projectskillselection.text().trim() == 'Breakdown: Skill') {
        ...
        for (i = 0; i < skills.length; i++) {
            var data_list = new Array(months.length).fill(0);
                ...
                if (prov[p[k]] != undefined) {
                    if (prov[p[k]][skills[i]] != undefined) {
                        ...
                        // if dollars is selected
                            ...
                            // for each of the people for this given skill
                            for (l = 0; l < prov[p[k]][skills[i]][2].length; l++) {

                                var person_salary_list = prov[p[k]][skills[i]][2][l];

                                sum_list = data_list.map(function(num, idx) {
        ...
        // required column data for dollars
            ...
            for (i = 0; i < skills.length; i++) {
                ...
                for (j = 0; j < month_indices.length; j++) {
                    sum_projects = 0;
                    for (k = 0; k < p.length; k++) {
                ...
                if (prov[p[i]] != undefined) {
                    if (prov[p[i]][skills[k]] != undefined) {
                        ...
                        if ($axesselection.text().trim() == 'Axes: FTE') {
                            ...
                            // for each of the people for this given skill
                            for (l = 0; l < prov[p[i]][skills[k]][1].length; l++) {
                            ...
                            // for each of the people for this given skill
                            for (l = 0; l < prov[p[i]][skills[k]][2].length; l++) {
    ...
    var chart = Highcharts.chart('total_chart', {
        chart: {
            type: 'column',
            options3d: {
                enabled: true,
        ...
        plotOptions: {
            ...
            series: {
                events: {
                    click: function(evt) {
                        var $highchartstooltip_tspan = $('.highcharts-tooltip tspan');
                        var tooltip_list = $highchartstooltip_tspan.text().split(';');

                        var query_string = '';
                        for (var i = 0; i < tooltip_list.length; i++) {
                            if (tooltip_list[i].includes(',')) {
                                var name = tooltip_list[i].split(':').slice(0, 1);
                                query_string += (name + ';');
                            }
                        }
...
$('#axes li a').click(function() {
    $axesselection.html($(this).text() + '<span class="caret"></span>');
    $axesselection.val($(this).text());
    $container.html('');
    if ($('#hypothetical-selection').text().trim() == 'Hypothetical On') {
        create_total_chart(projects, prov_overall_dict, req_overall_dict, skills, months);
    } else {
...
$('#projectskill li a').click(function() {
    $projectskillselection.html($(this).text() + '<span class="caret"></span>');
    $projectskillselection.val($(this).text());
    $container.html('');
    if ($('#hypothetical-selection').text().trim() == 'Hypothetical On') {
        create_total_chart(projects, prov_overall_dict, req_overall_dict, skills, months);
    } else {
